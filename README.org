...
=C-c H= opens a live table of agents (name, kind, status, cwd). On a row:
=RET= attach to that agent full-screen in eat, =s= send text, =f= focus,
=+= start an agent, =t= full multiplexer TUI, =g= refresh.
