...
enough; we also register it and grab focus. Alongside, pop a =magit-diff=
against the base ref showing which files were edited — this is =git diff
origin/master= (committed + uncommitted tracked changes), and unlike
=magit-status= it renders a single diff with no slow dashboard sections.
...
consult-grep) — they run against the workspace. In the diff buffer, =RET=
visits the file under point.
#+BEGIN_SRC emacs-lisp
(defvar open-workspace-base "origin/master"
  "Ref to diff a workspace against when listing edited files.")
...
\(projectile-find-file, consult-grep, ...) operate on it, and pop a
`magit-diff' of edited files against `open-workspace-base'."
  ...
  (let ((default-directory dir))
    (magit-diff-range open-workspace-base)))
#+END_SRC
