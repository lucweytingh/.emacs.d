...
(use-package monet
  :straight (:type git :host github :repo "stevemolitor/monet" :branch "main")
  :config
  (monet-mode 1))
...
;; Dynamically find claude CLI in common locations and add to PATH
(let ((claude-paths '("~/.nvm/versions/node/current/bin/claude"
                      "~/.nvm/versions/node/*/bin/claude"
                      "~/.local/bin/claude"
                      "/usr/local/bin/claude"
                      "/opt/homebrew/bin/claude")))
  (dolist (path-pattern claude-paths)
    (let ((expanded-path (expand-file-name path-pattern)))
      (if (string-match "\\*" path-pattern)
          ;; Handle glob patterns (like NVM version wildcards)
          (dolist (file (file-expand-wildcards expanded-path))
            (when (file-executable-p file)
              (let ((bin-dir (file-name-directory file)))
                (add-to-list 'exec-path bin-dir)
                (setenv "PATH" (concat (getenv "PATH") ":" bin-dir)))))
        ;; Handle direct paths
        (when (file-executable-p expanded-path)
          (let ((bin-dir (file-name-directory expanded-path)))
            (add-to-list 'exec-path bin-dir)
            (setenv "PATH" (concat (getenv "PATH") ":" bin-dir)))))))
