...
** herdr
Native overview for the [[https://herdr.dev][herdr]] agent runtime, driving a
running herdr server through its socket API (=our-herdr api/agent ...=).
Code lives in =lisp/herdr.el=; loaded lazily so it costs nothing until =C-c H=.

=C-c H= opens a live table of agents (name, kind, status, cwd) plus a small
help window listing every command (=?= reopens it). On a row: =RET= attach
to that agent full-screen in eat, =s= send text, =f= focus, =+= start an
agent, =t= full multiplexer TUI, =g= refresh.

=M-x herdr-tui= (or =t=) opens the whole herdr TUI in eat char mode: the
fork's =[emacs] enabled= layer means herdr itself speaks Emacs — =C-x=
chords for tabs/splits, =C-s= isearch, =C-y/M-y= kill ring, =C-x [= TEXT
mode. =M-RET= hands the keyboard back to Emacs. In any herdr buffer
=C-M-n= / =C-M-p= jump to the next/previous agent.
#+BEGIN_SRC emacs-lisp
(require 'server)
(unless (server-running-p) (server-start))

(add-to-list 'load-path (expand-file-name "lisp" user-emacs-directory))
(autoload 'herdr-status "herdr" "Open the herdr agent dashboard." t)
(autoload 'herdr-tui "herdr" "Open the herdr TUI in eat char mode." t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package eat
    ...
    ;; must be a 3-list (CURSOR-ON BLINK-FREQ CURSOR-OFF); a bare (bar . 2)
    ;; crashes eat's output timer on every cursor restyle (TUIs do this
    ;; constantly), freezing rendering mid-frame
    (setq eat-default-cursor-type '((bar . 2) nil nil))
