...
** mac
Install emacs 30 from https://emacsformacosx.com/
...
'(
  ...
  ("sitou" "/ssh:lucw@162.55.49.235:/home/lucw/" "(set-bash)")
  ("potato1" "/ssh:pi@192.168.0.111:/home/pi/" "(set-bash)")
  ("potato2" "/ssh:pi@192.168.0.121:/home/pi/" "(set-bash)")
  ("mirrors" "/ssh:pi@192.168.2.121:/home/pi/" "(set-bash)")
  ("pc" "/ssh:100.108.48.108:/" "(set-bash)")
 )
)
#+END_SRC
...
;; Keep native-compilation warnings out of the *Warnings* popup (still logged
;; to *Async-native-compile-log*). Use nil to mute fully.
...
(defun show-matching-paren-offscreen ()
...
(advice-add 'show-paren-function :after #'show-matching-paren-offscreen)
#+END_SRC
#+RESULTS:
: show-paren-function
...
(use-package counsel
  ...
  (define-key org-mode-map (kbd "C-c C-f") #'counsel-find-file))
  )
#+END_SRC
...
** dumb-jump (M-. everywhere)
Makes M-. (xref-find-definitions) work in any project without a TAGS
file or language server, using git-grep/grep under the hood.
...
(use-package embark-consult
  ...
  ;; Temporarily disable prot-embark to avoid loading issues
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;     (fill-paragraph nil)))
  ;; (global-set-key "\C-c e u" 'unfill-paragraph)
...
(defun open-clipboard-path ()
  ...
  (interactive)
  (let* ((paths (remove-if-not #'file-exists-p kill-ring))
...
(use-package string-inflection
  ...
  :config
  ;; Optional: your custom function using major-mode detection
  (defun my/string-inflection-cycle ()
    "Cycle naming style based on major mode."
    (interactive)
    (cond
...
(global-set-key (kbd "C-c s") 'string-inflection-underscore)
#+END_SRC

#+RESULTS:
: snake-case-symbol-after-point
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** move-file")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** copying current path to clipboard")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** duplicate region")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** jupyter insert token")
#+END_SRC
...
** set local directory
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** set local directory")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** shell from directory")
#+END_SRC
...
** monumental
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** monumental")
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun yarn-and-yarn-dev ()
  "Open a shell in a side window and run yarn && yarn dev."
  (interactive)
  (let* ((buffer-name "*yarn dev*")
        (default-directory "~/Documents/mech/portico/"))
    ;; Split window to the right
    (split-window-right)
    (other-window 1)
    ;; Create or switch to the yarn dev buffer
    (shell buffer-name)
    ;; Send the command to the shell
    (run-with-timer 0.5 nil
                    (lambda (buf-name)
                      (with-current-buffer buf-name
                        (comint-send-string (current-buffer) "yarn && yarn dev\n")))
                    buffer-name)))

(defun nix-pisa ()
  "Open external terminal and run nix pisa command."
  (interactive)
  (start-process "nix-pisa" nil "osascript" "-e"
                 "tell application \"Terminal\" to do script \"cd ~/Documents/mech/portico/ && nix run .#start-arcade -- --mock-standalone \\\"Pisa 2\\\" --speedup 5\""))

(defun nix-pisa-petra-panama ()
  "Open external terminal and run nix pisa petra panama command."
  (interactive)
  (start-process "nix-pisa-petra-panama" nil "osascript" "-e"
               "tell application \"Terminal\" to do script \"cd ~/Documents/mech/portico/ && nix run .#start-arcade -- --mock-standalone \\\"Pisa 2\\\" \\\"Petra 3\\\" \\\"Panama 1\\\" --speedup 5\""))
#+END_SRC

#+RESULTS:
: nix-pisa-petra-panama


** herdr
Native dashboard for the [[https://herdr.dev][herdr]] agent runtime. Drives a
running herdr server through its socket API (=our-herdr api/agent ...=) instead
of embedding herdr's TUI in a terminal, which renders poorly. Code lives in
=herdr.el=; loaded lazily so it costs nothing until =C-c H=.

=M-x herdr-status= opens a live list of panes/agents (workspace, status, cwd).
On a row: =RET= view output, =s= send text, =f= focus, =a= attach in a term,
=+= start an agent, =g= refresh.
#+BEGIN_SRC emacs-lisp
(require 'server)
(unless (server-running-p) (server-start))

(autoload 'herdr-status (expand-file-name "herdr" user-emacs-directory)
  "Open the herdr agent dashboard." t)
(global-set-key (kbd "C-c H") #'herdr-status)
#+END_SRC

** open workspace
Open a directory (e.g. a Conductor workspace) into the running frame with
projectile pointed at it, so search commands act on that project immediately.
Projectile derives the root from =default-directory=, so opening dired there is
enough; we also register it and grab focus. Alongside, pop =magit-status= for
the workspace so you can see which files were edited.

From a shell (bundle path since =emacsclient= isn't on PATH):
: cw() { /Applications/Emacs.app/Contents/MacOS/bin/emacsclient -e "(open-workspace \"${1:-$PWD}\")"; }
: cw /Users/luc/conductor/workspaces/mech/mock-build-no-registration
Then use your normal search keys (=C-x C-f= projectile-find-file, =M-s g=
consult-grep) — they run against the workspace.
#+BEGIN_SRC emacs-lisp
(defun open-workspace (dir)
  "Open DIR in this frame as the active projectile project.
Register DIR with projectile, focus the frame so search commands
\(projectile-find-file, consult-grep, ...) operate on it, and pop
`magit-status' to show which files were edited."
  (interactive "DWorkspace: ")
  (setq dir (file-name-as-directory (expand-file-name (string-trim dir))))
...
** dwim-shell-command
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** dwim shell command")
#+END_SRC
...
** open-specific-files
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** open specific files")
#+END_SRC
...
** find pattern in dir
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** find pattern in dir")
#+END_SRC
...
** recentf
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** recentf")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** yasnippet")
#+END_SRC
#+BEGIN_SRC emacs-lisp
...
** sudo remote
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** sudo remote")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** focus mode")
#+END_SRC
...
** claude code
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** claude code")
#+END_SRC

#+BEGIN_SRC emacs-lisp
  ;; Install monet for IDE integration with claude-code
  (use-package eat
    :straight (:type git
                     :host codeberg
                     :repo "akib/emacs-eat"
    ...
    :config
    ;; Prevent M-o from being sent to the terminal, let Emacs handle it
    (define-key eat-semi-char-mode-map (kbd "M-o") 'ace-window)
    (define-key eat-semi-char-mode-map (kbd "M-x") 'counsel-M-x)
    (define-key eat-semi-char-mode-map (kbd "C-x k") 'kill-buffer)
    (define-key eat-semi-char-mode-map (kbd "C-g") 'claude-code-ide-send-escape)
    (setq eat-default-cursor-type '(bar . 2))
  ...
  (use-package claude-code-ide
    ...
    (defun my/claude-code-insert-and-switch ()
      "Insert at-mentioned and switch to claude-code-ide buffer.
  If claude-code-ide is not running, start it first."
      (interactive)
      (if (get-buffer (claude-code-ide--get-buffer-name))
        ...
        (let ((source-buffer (current-buffer)))
          (claude-code-ide)
          ;; Use run-at-time to defer until eat finishes initial setup
          (run-at-time 0.5 nil
                       (lambda (buf)
                         (when (buffer-live-p buf)
                           ...
                           (claude-code-ide-switch-to-buffer)))
                       source-buffer))))
    (global-set-key (kbd "M-i") 'my/claude-code-insert-and-switch)
#+END_SRC
...
(use-package rust-mode
  ...
  :mode "\\.rs\\'"
  :config
  ;; Optional: customize rust-mode behavior here
  )
...
#+RESULTS:
| flycheck-rust-setup | rust-maybe-initialize-buffer-project |
...
** typescript
#+BEGIN_SRC emacs-lisp
(message "[README] Loading ** typescript mode")
#+END_SRC
...
(use-package typescript-mode
  ...
  :config
  (add-hook 'typescript-mode-hook (lambda () (company-mode))))
#+END_SRC

#+RESULTS:
: ((\.tsx\' . typescript-mode) (\.ts\' . typescript-mode) (\.tsx?\' . typescript-mode) (\.glsl\' . glsl-mode) (\.rmiss\' . glsl-mode) (\.rcall\' . glsl-mode) (\.rahit\' . glsl-mode) (\.rchit\' . glsl-mode) (\.rint\' . glsl-mode) (\.rgen\' . glsl-mode) (\.comp\' . glsl-mode) (\.task\' . glsl-mode) (\.mesh\' . glsl-mode) (\.tese\' . glsl-mode) (\.tesc\' . glsl-mode) (\.geom\' . glsl-mode) (\.frag\' . glsl-mode) (\.vert\' . glsl-mode) (\.js\' . js2-mode) (/Pipfile.lock$ . js2-mode) (/Pipfile$ . conf-mode) (\.py\' . python-mode) (\.ipynb$ . js2-mode) (\.tsv\' . tsv-mode) (\.[Cc][Ss][Vv]\' . csv-mode) (\.markdown\' . markdown-mode) (\.md\' . markdown-mode) (\.\(?:md\|markdown\|mkd\|mdown\|mkdn\|mdwn\)\' . markdown-mode) (\.html\' . html-mode) (/git-rebase-todo\' . git-rebase-mode) (\.pdf\' . pdf-view-mode) (\.gpg\(~\|\.~[0-9]+~\)?\' nil epa-file) (\.elc\' . elisp-byte-code-mode) (\.zst\' nil jka-compr) (\.dz\' nil jka-compr) (\.xz\' nil jka-compr) (\.lzma\' nil jka-compr) (\.lz\' nil jka-compr) (\.g?z\' nil jka-compr) (\.bz2\' nil jka-compr) (\.Z\' nil jka-compr) (\.vr[hi]?\' . vera-mode) (\(?:\.\(?:rbw?\|ru\|rake\|thor\|axlsx\|jbuilder\|rabl\|gemspec\|podspec\)\|/\(?:Gem\|Rake\|Cap\|Thor\|Puppet\|Berks\|Brew\|Fast\|Vagrant\|Guard\|Pod\)file\)\' . ruby-mode) (\.re?st\' . rst-mode) (/\(?:Pipfile\|\.?flake8\)\' . conf-mode) (\.py[iw]?\' . python-mode) (\.m\' . octave-maybe-mode) (\.less\' . less-css-mode) (\.editorconfig\' . editorconfig-conf-mode) (\.scss\' . scss-mode) (\.cs\' . csharp-mode) (\.awk\' . awk-mode) (\.\(u?lpc\|pike\|pmod\(\.in\)?\)\' . pike-mode) (\.idl\' . idl-mode) (\.java\' . java-mode) (\.m\' . objc-mode) (\.ii\' . c++-mode) (\.i\' . c-mode) (\.lex\' . c-mode) (\.y\(acc\)?\' . c-mode) (\.h\' . c-or-c++-mode) (\.c\' . c-mode) (\.\(CC?\|HH?\)\' . c++-mode) (\.[ch]\(pp\|xx\|\+\+\)\' . c++-mode) (\.\(cc\|hh\)\' . c++-mode) (\.\(bat\|cmd\)\' . bat-mode) (\.[sx]?html?\(\.[a-zA-Z_]+\)?\' . mhtml-mode) (\.svgz?\' . image-mode) (\.svgz?\' . xml-mode) (\.x[bp]m\' . image-mode) (\.x[bp]m\' . c-mode) (\.p[bpgn]m\' . image-mode) (\.tiff?\' . image-mode) (\.gif\' . image-mode) (\.png\' . image-mode) (\.jpe?g\' . image-mode) (\.webp\' . image-mode) (\.te?xt\' . text-mode) (\.[tT]e[xX]\' . tex-mode) (\.ins\' . tex-mode) (\.ltx\' . latex-mode) (\.dtx\' . doctex-mode) (\.org\' . org-mode) (\.dir-locals\(?:-2\)?\.el\' . lisp-data-mode) (\.eld\' . lisp-data-mode) (eww-bookmarks\' . lisp-data-mode) (tramp\' . lisp-data-mode) (/archive-contents\' . lisp-data-mode) (places\' . lisp-data-mode) (\.emacs-places\' . lisp-data-mode) (\.el\' . emacs-lisp-mode) (Project\.ede\' . emacs-lisp-mode) (\(?:\.\(?:scm\|sls\|sld\|stk\|ss\|sch\)\|/\.guile\)\' . scheme-mode) (\.l\' . lisp-mode) (\.li?sp\' . lisp-mode) (\.[fF]\' . fortran-mode) (\.for\' . fortran-mode) (\.p\' . pascal-mode) (\.pas\' . pascal-mode) (\.\(dpr\|DPR\)\' . delphi-mode) (\.\([pP]\([Llm]\|erl\|od\)\|al\)\' . perl-mode) (Imakefile\' . makefile-imake-mode) (Makeppfile\(?:\.mk\)?\' . makefile-makepp-mode) (\.makepp\' . makefile-makepp-mode) (\.mk\' . makefile-bsdmake-mode) (\.make\' . makefile-bsdmake-mode) (GNUmakefile\' . makefile-gmake-mode) ([Mm]akefile\' . makefile-bsdmake-mode) (\.am\' . makefile-automake-mode) (\.texinfo\' . texinfo-mode) (\.te?xi\' . texinfo-mode) (\.[sS]\' . asm-mode) (\.asm\' . asm-mode) (\.css\' . css-mode) (\.mixal\' . mixal-mode) (\.gcov\' . compilation-mode) (/\.[a-z0-9-]*gdbinit . gdb-script-mode) (-gdb\.gdb . gdb-script-mode) ([cC]hange\.?[lL]og?\' . change-log-mode) ([cC]hange[lL]og[-.][0-9]+\' . change-log-mode) (\$CHANGE_LOG\$\.TXT . change-log-mode) (\.scm\.[0-9]*\' . scheme-mode) (\.[ckz]?sh\'\|\.shar\'\|/\.z?profile\' . sh-mode) (\.bash\' . sh-mode) (/PKGBUILD\' . sh-mode) (\(/\|\`\)\.\(bash_\(profile\|history\|log\(in\|out\)\)\|z?log\(in\|out\)\)\' . sh-mode) (\(/\|\`\)\.\(shrc\|zshrc\|m?kshrc\|bashrc\|t?cshrc\|esrc\)\' . sh-mode) (\(/\|\`\)\.\([kz]shenv\|xinitrc\|startxrc\|xsession\)\' . sh-mode) (\.m?spec\' . sh-mode) (\.m[mes]\' . nroff-mode) (\.man\' . nroff-mode) (\.sty\' . latex-mode) (\.cl[so]\' . latex-mode) (\.bbl\' . latex-mode) (\.bib\' . bibtex-mode) (\.bst\' . bibtex-style-mode) (\.sql\' . sql-mode) (\(acinclude\|aclocal\|acsite\)\.m4\' . autoconf-mode) (\.m[4c]\' . m4-mode) (\.mf\' . metafont-mode) (\.mp\' . metapost-mode) (\.vhdl?\' . vhdl-mode) (\.article\' . text-mode) (\.letter\' . text-mode) (\.i?tcl\' . tcl-mode) (\.exp\' . tcl-mode) (\.itk\' . tcl-mode) (\.icn\' . icon-mode) (\.sim\' . simula-mode) (\.mss\' . scribe-mode) (\.f9[05]\' . f90-mode) (\.f0[38]\' . f90-mode) (\.indent\.pro\' . fundamental-mode) (\.\(pro\|PRO\)\' . idlwave-mode) (\.srt\' . srecode-template-mode) (\.prolog\' . prolog-mode) (\.tar\' . tar-mode) (\.\(arc\|zip\|lzh\|lha\|zoo\|[jew]ar\|xpi\|rar\|cbr\|7z\|squashfs\|ARC\|ZIP\|LZH\|LHA\|ZOO\|[JEW]AR\|XPI\|RAR\|CBR\|7Z\|SQUASHFS\)\' . archive-mode) (\.oxt\' . archive-mode) (\.\(deb\|[oi]pk\)\' . archive-mode) (\`/tmp/Re . text-mode) (/Message[0-9]*\' . text-mode) (\`/tmp/fol/ . text-mode) (\.oak\' . scheme-mode) (\.sgml?\' . sgml-mode) (\.x[ms]l\' . xml-mode) (\.dbk\' . xml-mode) (\.dtd\' . sgml-mode) (\.ds\(ss\)?l\' . dsssl-mode) (\.js[mx]?\' . javascript-mode) (\.har\' . javascript-mode) (\.json\' . js-json-mode) (\.[ds]?va?h?\' . verilog-mode) (\.by\' . bovine-grammar-mode) (\.wy\' . wisent-grammar-mode) (\.erts\' . erts-mode) ([:/\]\..*\(emacs\|gnus\|viper\)\' . emacs-lisp-mode) (\`\..*emacs\' . emacs-lisp-mode) ([:/]_emacs\' . emacs-lisp-mode) (/crontab\.X*[0-9]+\' . shell-script-mode) (\.ml\' . lisp-mode) (\.ld[si]?\' . ld-script-mode) (ld\.?script\' . ld-script-mode) (\.xs\' . c-mode) (\.x[abdsru]?[cnw]?\' . ld-script-mode) (\.zone\' . dns-mode) (\.soa\' . dns-mode) (\.asd\' . lisp-mode) (\.\(asn\|mib\|smi\)\' . snmp-mode) (\.\(as\|mi\|sm\)2\' . snmpv2-mode) (\.\(diffs?\|patch\|rej\)\' . diff-mode) (\.\(dif\|pat\)\' . diff-mode) (\.[eE]?[pP][sS]\' . ps-mode) (\.\(?:PDF\|EPUB\|CBZ\|FB2\|O?XPS\|DVI\|OD[FGPST]\|DOCX\|XLSX?\|PPTX?\|pdf\|epub\|cbz\|fb2\|o?xps\|djvu\|dvi\|od[fgpst]\|docx\|xlsx?\|pptx?\)\' . doc-view-mode-maybe) (configure\.\(ac\|in\)\' . autoconf-mode) (\.s\(v\|iv\|ieve\)\' . sieve-mode) (BROWSE\' . ebrowse-tree-mode) (\.ebrowse\' . ebrowse-tree-mode) (#\*mail\* . mail-mode) (\.g\' . antlr-mode) (\.mod\' . m2-mode) (\.ses\' . ses-mode) (\.docbook\' . sgml-mode) (\.com\' . dcl-mode) (/config\.\(?:bat\|log\)\' . fundamental-mode) (/\.?\(authinfo\|netrc\)\' . authinfo-mode) (\.\(?:[iI][nN][iI]\|[lL][sS][tT]\|[rR][eE][gG]\|[sS][yY][sS]\)\' . conf-mode) (\.la\' . conf-unix-mode) (\.ppd\' . conf-ppd-mode) (java.+\.conf\' . conf-javaprop-mode) (\.properties\(?:\.[a-zA-Z0-9._-]+\)?\' . conf-javaprop-mode) (\.toml\' . conf-toml-mode) (\.desktop\' . conf-desktop-mode) (/\.redshift\.conf\' . conf-windows-mode) (\`/etc/\(?:DIR_COLORS\|ethers\|.?fstab\|.*hosts\|lesskey\|login\.?de\(?:fs\|vperm\)\|magic\|mtab\|pam\.d/.*\|permissions\(?:\.d/.+\)?\|protocols\|rpc\|services\)\' . conf-space-mode) (\`/etc/\(?:acpid?/.+\|aliases\(?:\.d/.+\)?\|default/.+\|group-?\|hosts\..+\|inittab\|ksysguarddrc\|opera6rc\|passwd-?\|shadow-?\|sysconfig/.+\)\' . conf-mode) ([cC]hange[lL]og[-.][-0-9a-z]+\' . change-log-mode) (/\.?\(?:gitconfig\|gnokiirc\|hgrc\|kde.*rc\|mime\.types\|wgetrc\)\' . conf-mode) (/\.mailmap\' . conf-unix-mode) (/\.\(?:asound\|enigma\|fetchmail\|gltron\|gtk\|hxplayer\|mairix\|mbsync\|msmtp\|net\|neverball\|nvidia-settings-\|offlineimap\|qt/.+\|realplayer\|reportbug\|rtorrent\.\|screen\|scummvm\|sversion\|sylpheed/.+\|xmp\)rc\' . conf-mode) (/\.\(?:gdbtkinit\|grip\|mpdconf\|notmuch-config\|orbital/.+txt\|rhosts\|tuxracer/options\)\' . conf-mode) (/\.?X\(?:default\|resource\|re\)s\> . conf-xdefaults-mode) (/X11.+app-defaults/\|\.ad\' . conf-xdefaults-mode) (/X11.+locale/.+/Compose\' . conf-colon-mode) (/X11.+locale/compose\.dir\' . conf-javaprop-mode) (\.~?[0-9]+\.[0-9][-.0-9]*~?\' nil t) (\.\(?:orig\|in\|[bB][aA][kK]\)\' nil t) ([/.]c\(?:on\)?f\(?:i?g\)?\(?:\.[a-zA-Z0-9._-]+\)?\' . conf-mode-maybe) (\.[1-9]\' . nroff-mode) (\.art\' . image-mode) (\.avs\' . image-mode) (\.bmp\' . image-mode) (\.cmyk\' . image-mode) (\.cmyka\' . image-mode) (\.crw\' . image-mode) (\.dcr\' . image-mode) (\.dcx\' . image-mode) (\.dng\' . image-mode) (\.dpx\' . image-mode) (\.fax\' . image-mode) (\.heic\' . image-mode) (\.hrz\' . image-mode) (\.icb\' . image-mode) (\.icc\' . image-mode) (\.icm\' . image-mode) (\.ico\' . image-mode) (\.icon\' . image-mode) (\.jbg\' . image-mode) (\.jbig\' . image-mode) (\.jng\' . image-mode) (\.jnx\' . image-mode) (\.miff\' . image-mode) (\.mng\' . image-mode) (\.mvg\' . image-mode) (\.otb\' . image-mode) (\.p7\' . image-mode) (\.pcx\' . image-mode) (\.pdb\' . image-mode) (\.pfa\' . image-mode) (\.pfb\' . image-mode) (\.picon\' . image-mode) (\.pict\' . image-mode) (\.rgb\' . image-mode) (\.rgba\' . image-mode) (\.tga\' . image-mode) (\.wbmp\' . image-mode) (\.webp\' . image-mode) (\.wmf\' . image-mode) (\.wpg\' . image-mode) (\.xcf\' . image-mode) (\.xmp\' . image-mode) (\.xwd\' . image-mode) (\.yuv\' . image-mode) (\.tgz\' . tar-mode) (\.tbz2?\' . tar-mode) (\.txz\' . tar-mode) (\.tzst\' . tar-mode))
...
;;   :config
;;   (progn
;;     ;; Delay server installation to ensure irony is fully loaded
;;     (run-with-idle-timer 2 nil
;;       (lambda ()
;;         (when (and (fboundp 'irony--find-server-executable)
;;                    (fboundp 'irony-install-server))
;;           ;; Check if irony server is installed, if not, try to install it automatically
;;           (unless (irony--find-server-executable)
;;             (message "Irony server not found. Attempting to install...")
;;             (condition-case err
;;                 (irony-install-server)
;;               (error
;;                (message "Failed to install irony server automatically: %s" err)
;;                (message "Please run M-x irony-install-server manually"))))))
...
;; ;; Helper function to install irony server
;; (defun install-irony-server-if-needed ()
;;   "Install irony server if it's not already installed."
;;   (interactive)
;;   (if (irony--find-server-executable)
;;       (message "Irony server is already installed.")
;;     (message "Installing irony server...")
;;     (irony-install-server)))

;; ;; Bind the function to a convenient key
;; (global-set-key (kbd "C-c i") 'install-irony-server-if-needed)
#+END_SRC
...
* workspace utils
Per-project development tooling ported from [[https://github.com/fayalalebrun/.emacs.d][fayalalebrun's config]].
A "workspace" == the current Projectile project. Provides:
- tag-scoped [[https://github.com/rejeep/prodigy.el][prodigy]] services per project (idempotent setup/teardown),
- one dedicated IPython REPL per notebook (=code-cells=),
- a command hydra on =C-c W= (=C-c m= is taken by =avy-move-line= here),
- monorepo commands (proto, pyright, mock robots, arduino, format, ...).

The source lives in [[file:lisp/workspace-utils.el][lisp/workspace-utils.el]]. Note: the monorepo commands
assume Francisco's project layout (=portico/=, =arcade/=, =vision/=, =ctrl/only/=)
and =nix=; the reusable pieces (prodigy services, notebook REPLs, hydra) work
anywhere. yarn was replaced by =pnpm=. Machine-specific paths are now defcustoms
(=workspace-grpcwebproxy-path=, =workspace-atrium-directory=,
=workspace-ipython-extra-path=).

** dependencies
=projectile= and =eat= are configured earlier; these are the extra ones.
#+BEGIN_SRC emacs-lisp
(use-package prodigy :straight t)
(use-package hydra :straight t)
(use-package code-cells :straight t :defer t)
#+END_SRC

** load
=workspace-utils= requires =prodigy= and =hydra= at load time, so this block
must come after the package block above.
#+BEGIN_SRC emacs-lisp
