...
;;   "Highlight CSV columns with different colors, applying only to column content."
...
;;              for c in colors
;;              for r = (format "^\\(?:[^%c\n]+%c\\)\\{%d\\}\\([^%c\n]+\\)" separator separator (1- i) separator)
...
#+RESULTS:
| (lambda nil (interactive) (toggle-truncate-lines nil)) | csv-align-mode | csv-highlight |
